...
{
    public class IsCustomerHandler : AuthorizationHandler<HasAccessToCustomers>
    {
        protected UnitOfWork Unit;
        public IsCustomerHandler(TimeKeeperContext context)
        {
            Unit = new UnitOfWork(context);
        }

        protected override Task HandleRequirementAsync(AuthorizationHandlerContext context, HasAccessToCustomers requirement)
        {
            ...
            var teams = Unit.Employees.Get(empId).Memberships.GroupBy(x => x.Team.Id).Select(y => y.Key).ToList();
