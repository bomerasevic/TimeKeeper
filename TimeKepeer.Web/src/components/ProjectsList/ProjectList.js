...
let counter = 0;
function createData(name, team, customer, status) {
    counter += 1;
    return { id: counter, name, team, customer, status };
}
...
const rows = [
    ...
    { id: "team", numeric: true, disablePadding: true, label: "Customer name" },
    { id: "customer", numeric: false, disablePadding: false, label: "Team Name" },
        ...
        axios(`${config.apiUrl}projects`, {
            ...
            .then(res => {
                ...
                let fetchedData = res.data.map(r =>
                    createData(r.name, r.team, r.customer, r.status)
                );
